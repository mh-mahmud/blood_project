                ...
                <div class="donor-search-form__field">
                    ...
                    <select class="select" name="city_id">
                        ...
                        @foreach ($cities as $city)
                            <option value="{{ $city->id }}" data-locations="{{ json_encode($city->locations) }}">
                                {{ __($city->name) }}</option>
                        @endforeach
                    </select>
                </div>
                            ...
                            <div class="col-lg-6 col-md-12 col-sm-6">
                                <div class="donor-item has--link">
                                    ...
                                    <div class="donor-item__thumb">
                                        <img src="{{ getImage('assets/images/donor/' . $donor->image, getFilePath('donor')) }}"
                                            alt="@lang('image')">
                                    </div>
                                    <div class="donor-item__content">
                                        ...
                                        <ul class="donor-item__list mt-2">
                                            ...
                                            <li>
                                                <i class="las la-tint"></i>@lang('Blood Group') : <span
                                                    class="text--base">({{ __($donor->blood->name) }})</span>
                                            </li>
                                        </ul>
                                        <a href="{{ route('donor.details', [slug($donor->name), encrypt($donor->id)]) }}"
                                            class="text--base fs--14px text-decoration-underline">@lang('View Profile')</a>
                                    </div>
                                </div>
                            </div>
                        @empty
